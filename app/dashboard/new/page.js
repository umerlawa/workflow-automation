...
import Link from 'next/link'                     // ← added for compliance link
...
export default function WorkflowBuilder() {
  ...
  const [showComplianceWarning, setShowComplianceWarning] = useState(false)  // ← added
  ...
  const checkFinancialKeywords = (text) => {
    const financialKeywords = ['payment', 'transaction', 'transfer', 'invoice', 'billing', 'money', '$', 'dollar', 'financial', 'bank']
    const healthcareKeywords = ['patient', 'medical', 'health', 'prescription', 'diagnosis', 'treatment']
    const lower = text.toLowerCase()
    const hasFinancial = financialKeywords.some(k => lower.includes(k))
    const hasHealthcare = healthcareKeywords.some(k => lower.includes(k))
    return { hasFinancial, hasHealthcare }
  }
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <textarea
          ...
          onChange={(e) => {
            ...
            setDescription(txt)
            const { hasFinancial, hasHealthcare } = checkFinancialKeywords(txt)
            setShowComplianceWarning(hasFinancial || hasHealthcare)
          }}
        ...
        {showComplianceWarning && (
          <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            <p className="text-sm text-yellow-800">
              <strong>⚠️ Compliance Notice:</strong> Financial workflows involving transactions above $500 
              and healthcare workflows require human oversight per{' '}
              <Link href="/tos#section-6-2" className="text-yellow-900 underline">
                Section 6.2 of our Terms of Service
              </Link>.
            </p>
          </div>
        )}
